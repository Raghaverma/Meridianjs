...
- **Stripe** ([`src/providers/payments/stripe/adapter.ts`](../src/providers/payments/stripe/adapter.ts)) — every `429` is `rate_limit` and `retryable: true`. Stripe's `Retry-After` is meaningful, so it's parsed and attached to the error as `retryAfter`.
- **OpenAI** ([`src/providers/ai/openai/adapter.ts`](../src/providers/ai/openai/adapter.ts)) — `429` is `rate_limit`, but `retryable` is `false` when the error body's `code` is `insufficient_quota`. That's a billing wall, not a transient throttle — retrying it just burns attempts on a request that can never succeed until the account is topped up.
- **GitHub** ([`src/providers/crm/github/adapter.ts`](../src/providers/crm/github/adapter.ts)) — GitHub's *secondary* rate limit is signaled as `403` with an `X-RateLimit-Remaining: 0` header, not `429` at all. The adapter still classifies it as `category: "rate_limit"`, `retryable: true`, because the signal matters more than the status code that carried it.
- **Anthropic** ([`src/providers/ai/anthropic/adapter.ts`](../src/providers/ai/anthropic/adapter.ts)) — capacity/overload conditions use a distinct `529` status, kept under `category: "provider"` rather than `rate_limit`, since it's a capacity signal rather than a quota one (and carries no `Retry-After`).
