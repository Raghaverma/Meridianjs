...
export const Fixtures = {
  razorpay: {
    order: (overrides?: Record<string, unknown>): MockResponse => ({
      status: 200,
      body: { id: "order_mock123", entity: "order", amount: 50000, currency: "INR", status: "created", ...overrides },
    }),
    payment: (overrides?: Record<string, unknown>): MockResponse => ({
      status: 200,
      body: { id: "pay_mock123", entity: "payment", amount: 50000, currency: "INR", status: "captured", ...overrides },
    }),
  ...
  cashfree: {
    order: (overrides?: Record<string, unknown>): MockResponse => ({
      status: 200,
      body: { cf_order_id: "mock_cf_123", order_id: "order_mock", order_status: "ACTIVE", ...overrides },
    }),
  },
  ...
  stripe: {
    paymentIntent: (overrides?: Record<string, unknown>): MockResponse => ({
      status: 200,
      body: { id: "pi_mock123", object: "payment_intent", amount: 1000, currency: "usd", status: "succeeded", ...overrides },
    }),
  },
  ...
  generic: {
    ...
    serverError: (message = "Internal Server Error"): MockResponse => ({ status: 500, body: { message } }),
