...
export class DecentroAdapter implements ProviderAdapter {
  ...
  buildRequest(input: AdapterInput): BuiltRequest {
    ...
    // Token is encoded as "clientId|clientSecret|moduleSecret"
  ...
  private parseHttpError(error: {
    ...
    const errorBody = body as DecentroErrorBody | undefined;
  ...
  rateLimitPolicy(_headers: Headers): RateLimitInfo {
    return { limit: 100, remaining: 100, reset: new Date(Date.now() + 60_000) };
  }

  paginationStrategy(): PaginationStrategy {
    return new DecentroPaginationStrategy();
  }
