...
export class CohereAdapter implements ProviderAdapter {
  ...
  buildRequest(input: AdapterInput): BuiltRequest {
    ...
    const baseUrlStripped = effectiveBaseUrl.endsWith("/") ? effectiveBaseUrl.slice(0, -1) : effectiveBaseUrl;
  ...
  private parseHttpError(error: { status: number; headers?: Headers | Record<string, string>; body?: unknown }): MeridianError {
    ...
    if (status === 422) return this.createError("validation", false, message || "Unprocessable Entity", meta, undefined, 422);
    if (status === 429) {
    ...
    if (status >= 500) return this.createError("provider", true, message || `Cohere API error ${status}`, meta, undefined, status);
  ...
  async authStrategy(config: AuthConfig): Promise<AuthToken> {
    ...
    if (!key) throw this.createError("auth", false, "Cohere requires an API key. Set auth.apiKey.", {}, undefined, 401);
    return { token: key };
  }
  ...
  getIdempotencyConfig(): IdempotencyConfig {
    return { defaultSafeOperations: new Set(["GET", "HEAD", "OPTIONS"]), operationOverrides: new Map() };
  }

  verifyWebhook(_payload: string | Buffer | Record<string, unknown>, _signature: string, _secret: string): boolean {
  ...
  private createError(category: MeridianError["category"], retryable: boolean, message: string, metadata?: Record<string, unknown>, retryAfter?: Date, status?: number): MeridianError {
    return new MeridianError(message, category, "cohere", retryable, "", metadata, retryAfter, status);
  }
