...
export class ShiprocketAdapter implements ProviderAdapter {
  ...
  constructor(baseUrl: string = "https://apiv2.shiprocket.in") {
  ...
  parseResponse(raw: RawResponse): NormalizedResponse {
    ...
    return ResponseNormalizer.normalize(raw, "shiprocket", rateLimitInfo, paginationInfo, [], "1.0.0");
  }
  ...
  private parseHttpError(error: {
    ...
    if (status === 400 || status === 422)
      return this.createMeridianError("validation", false, errorMessage ?? "Validation failed.", { errors: errorBody?.errors }, undefined, status);
    if (status >= 500)
      return this.createMeridianError("provider", true, `Shiprocket API returned error ${status}.`, { status }, undefined, status);
    if (status >= 400)
  ...
  async authStrategy(config: AuthConfig): Promise<AuthToken> {
    ...
    if (!token) throw this.createMeridianError("auth", false, "Requires token or apiKey (pre-obtained JWT).", {}, undefined, 401);
    return { token };
  }
  ...
  private createMeridianError(
    ...
    return new MeridianError(message, category, "shiprocket", retryable, "", metadata, retryAfter, status);
  }
