...
export class MailgunAdapter implements ProviderAdapter {
  ...
  constructor(baseUrl = "https://api.mailgun.net") {
  ...
  buildRequest(input: AdapterInput): BuiltRequest {
    ...
    const credentials = Buffer.from(`api:${authToken.token}`).toString("base64");
  ...
  parseResponse(raw: RawResponse): NormalizedResponse {
    ...
    return ResponseNormalizer.normalize(raw, "mailgun", rateLimitInfo, paginationInfo, [], "1.0.0");
  }
  ...
  private parseHttpError(error: {
    ...
    if (typeof body === "object" && body !== null) {
      ...
      mailgunMessage = String(b.message ?? b.error ?? "");
    } else if (typeof body === "string") {
      mailgunMessage = body;
    }

    const metadata = {
      mailgunError: mailgunMessage,
    };
    ...
    if (status === 401 || status === 403) {
      return this.createMeridianError(
        "auth",
        false,
        mailgunMessage || (status === 401 ? "Unauthorized" : "Forbidden"),
        metadata,
        undefined,
        status,
      );
    ...
    if (status === 429) {
      ...
      return this.createMeridianError(
        "rate_limit",
        true,
        mailgunMessage || "Rate limit exceeded.",
    ...
    if (status >= 500) {
      return this.createMeridianError(
        "provider",
        true,
        mailgunMessage || `Mailgun API error ${status}`,
        metadata,
        undefined,
        status,
      );
    ...
    return this.createMeridianError(
      "validation",
      false,
      mailgunMessage || `Request failed with status ${status}`,
      metadata,
      undefined,
      status,
    );
  ...
  async authStrategy(config: AuthConfig): Promise<AuthToken> {
    ...
    if (!key) {
      throw this.createMeridianError(
        "auth",
        false,
        "Mailgun authentication requires an API Key. Set auth.apiKey or auth.token.",
        {},
        undefined,
        401,
      );
  ...
  paginationStrategy(): PaginationStrategy {
    return new MailgunPaginationStrategy();
  }
  ...
  verifyWebhook(
    ...
    try {
      ...
      const sigObj = body.signature as Record<string, unknown> | undefined;
      if (!sigObj) {
        return false;
      }

      const timestamp = String(sigObj.timestamp ?? "");
      const token = String(sigObj.token ?? "");
      const providedSignature = signature || String(sigObj.signature ?? "");
      ...
      const bufB = Buffer.from(providedSignature);
