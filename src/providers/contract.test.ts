 ...
 *
 * Every built-in adapter registered in `BUILTIN_ADAPTER_CLASSES` is run through
...
describe("Provider Contracts (all registered adapters)", () => {
  ...
  let entries = Object.entries(BUILTIN_ADAPTER_CLASSES).sort(([a], [b]) => a.localeCompare(b));

  if (only) {
    entries = entries.filter(([name]) => name === only);
    if (entries.length === 0) {
      const known = Object.keys(BUILTIN_ADAPTER_CLASSES).sort().join(", ");
