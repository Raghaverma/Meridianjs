...
import { SentryPaginationStrategy } from "./pagination.js";
 ...
 */
export class SentryAdapter implements ProviderAdapter {
  private baseUrl: string;

  constructor(baseUrl = "https://sentry.io/api/0/") {
  ...
  parseResponse(raw: RawResponse): NormalizedResponse {
    ...
    return ResponseNormalizer.normalize(raw, "sentry", rateLimitInfo, paginationInfo, [], "1.0.0");
  }
  ...
  private parseHttpError(error: {
    ...
    const errorBody = body as SentryErrorBody | undefined;
    const errorMessage = errorBody?.detail ?? errorBody?.error;

    if (status === 401) {
      return this.createMeridianError(
        "auth",
        false,
        errorMessage ?? "Authentication failed. Check your Sentry auth token.",
        { sentryError: errorBody },
        undefined,
        401,
      );
    }
    ...
    if (status === 403) {
      return this.createMeridianError(
        "auth",
        false,
        errorMessage ?? "Permission denied. Your auth token lacks the required scopes.",
        { sentryError: errorBody },
        undefined,
        403,
      );
    ...
    if (status >= 500) {
      return this.createMeridianError(
        "provider",
        true,
        errorMessage ?? `Sentry API returned error ${status}. This may be temporary.`,
        { status },
        undefined,
        status,
      );
  ...
  async authStrategy(config: AuthConfig): Promise<AuthToken> {
    ...
    if (!token) {
      throw this.createMeridianError(
        "auth",
        false,
        "Sentry authentication requires an auth token. Set auth.token (internal integration " +
          "token or personal API token with the appropriate scopes).",
        {},
        undefined,
        401,
      );
  ...
  paginationStrategy(): PaginationStrategy {
    return new SentryPaginationStrategy();
  }
