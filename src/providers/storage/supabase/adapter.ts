...
export class SupabaseAdapter implements ProviderAdapter {
  ...
  constructor(baseUrl = "https://project.supabase.co/rest/v1") {
  ...
  private parseHttpError(error: {
    ...
    if (typeof body === "object" && body !== null) {
      ...
      supabaseMessage = String(b.message ?? b.error_description ?? b.error ?? b.hint ?? "");
    } else if (typeof body === "string") {
      supabaseMessage = body;
    }

    const metadata = { supabaseError: supabaseMessage };

    if (status === 401 || status === 403) {
      return this.createMeridianError(
        "auth",
        false,
        supabaseMessage || (status === 401 ? "Unauthorized" : "Forbidden"),
        metadata,
        undefined,
        status,
      );
    ...
    if (status === 429) {
      ...
      return this.createMeridianError(
        "rate_limit",
        true,
        supabaseMessage || "Rate limit exceeded.",
    ...
    if (status === 400 || status === 422) {
      return this.createMeridianError(
        "validation",
        false,
        supabaseMessage || "Validation failed.",
        metadata,
        undefined,
        status,
      );
    ...
    if (status >= 500) {
      return this.createMeridianError(
        "provider",
        true,
        supabaseMessage || `Supabase API error ${status}`,
        metadata,
        undefined,
        status,
      );
    ...
    return this.createMeridianError(
      "validation",
      false,
      supabaseMessage || `Request failed with status ${status}`,
      metadata,
      undefined,
      status,
    );
  ...
  async authStrategy(config: AuthConfig): Promise<AuthToken> {
    ...
    if (!key) {
      throw this.createMeridianError(
        "auth",
        false,
        "Supabase authentication requires an API Key. Set auth.apiKey or auth.token.",
        {},
        undefined,
        401,
      );
  ...
  paginationStrategy(): PaginationStrategy {
    return new SupabasePaginationStrategy();
  }
