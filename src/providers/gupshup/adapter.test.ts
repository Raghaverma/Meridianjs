...
describe("GupshupAdapter - Contract Tests", () => {
  ...
  describe("parseResponse", () => {
    it("should normalize a successful response", () => {
      const raw: RawResponse = { status: 200, headers: new Headers(), body: { status: "submitted", messageId: "msg_123" } };
  ...
  describe("parseError", () => {
    it("should map 401 to auth category", () => {
      const error = adapter.parseError({ status: 401, headers: new Headers(), body: { status: "error", message: "Invalid API Key" } });
