...
describe("PerfiosAdapter - Contract Tests", () => {
  ...
  describe("parseResponse", () => {
    it("should normalize a successful response", () => {
      const raw: RawResponse = { status: 200, headers: new Headers(), body: { txnId: "txn_123", status: "INITIATED" } };
  ...
  describe("parseError", () => {
    it("should map 401 to auth category", () => {
      const error = adapter.parseError({ status: 401, headers: new Headers(), body: { error: "Unauthorized", message: "Invalid API key" } });
